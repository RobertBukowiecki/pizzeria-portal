...
const renderActions = (status, id) => {
  switch (status) {
    case 'null':
      return (
        <>
          <Button style={{margin: '0 10px', backgroundColor: 'rgb(250, 250, 250)'}} variant="outlined" component={Link} to={`tables/booking/new`}>New booking</Button>
          <Button style={{margin: '0 10px', backgroundColor: 'rgb(250, 250, 250)'}} variant="outlined" component={Link} to={`tables/event/new`}>New event</Button>
        </>
      );
    case 'booked':
      return (
        <Button style={{margin: '0 10px', backgroundColor: 'rgb(250, 250, 250)'}} variant="outlined" component={Link} to={`/tables/bookings/${id}`}>Booking Details</Button>
      );
    case 'event':
      return (
        <Button style={{margin: '0 10px', backgroundColor: 'rgb(250, 250, 250)'}}variant="outlined" component={Link} to={`/tables/events/${id}`}>Event Details</Button>
      );
    default:
      return null;
  }
  ...
  return (
    <div className={styles.component}>
      ...
      <div className={styles.legend}>
        ...
        <div><span className={styles.eventLegend}></span><span> - Event table</span></div>
      </div>
    </div>
  );
};
